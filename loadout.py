...
Selecting `attachment=roarm2` only records the profile. It does **not**
import or start RoArm drivers — those stay out of this rover-first tree.
"""
...
ATTACHMENTS: dict[str, dict[str, Any]] = {
    ...
    "roarm2": {
        ...
        "label": "RoArm-M2",
        "module_type": 1,
        # Not wired on this consolidate build. UI may show the stencil + banner.
        "wired": False,
    },
}
...
ARM_OFFLINE_MESSAGE = "arm offline — not wired on this build yet"
...
def effective_types(loadout: Mapping[str, Any]) -> dict[str, Any]:
    ...
    arm_offline = lo["attachment"] == "roarm2" and not att.get("wired", False)
    return {
        ...
        "arm_message": ARM_OFFLINE_MESSAGE if arm_offline else None,
    }
...
def apply_loadout_to_config(
    ...
    Never writes `drive_linear_sign`, `drive_angular_sign`, or `arm_config`.
    """
...
def public_payload(
    ...
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "roarm_started": False,
        "note": (
            "drive signs stay in config.yaml; changing loadout does not start RoArm"
        ),
        **catalog_payload(),
        **drive_signs_from_config(cfg),
    }
    if types["arm_offline"]:
        payload["arm_status"] = "offline"
        payload["arm_message"] = ARM_OFFLINE_MESSAGE
    else:
        payload["arm_status"] = "n/a" if lo["attachment"] == "none" else "ready"
        payload["arm_message"] = None
    return payload


class LoadoutStore:
    """Thread-safe `.loadout.json` store. No hardware side effects."""
    ...
    def set(self, patch: Optional[Mapping[str, Any]] = None) -> dict[str, Any]:
        ...
        Never starts RoArm (or any other) drivers.
        """
        current = self.get()
        if patch:
    ...
    def public(self, cfg: Optional[Mapping[str, Any]] = None) -> dict[str, Any]:
